...
public class InputCheck {
     ...
     * @return
     */
    public static boolean cbListCheck(LinkedList<CheckBox> cbList) {
    ...
    /**
     * 监听EditText是否为全非空及CheckBox是否全被选中
     *
     * @param needCb
     */
    public static boolean checkEtAndCbList(boolean needCb, LinkedList<EditText> edList, LinkedList<CheckBox> cbList) {
        if (needCb) {
            if (!edListCheck(edList) || !cbListCheck(cbList)) {
                return false;
            }
            return true;
        }
        return edListCheck(edList);
    }
