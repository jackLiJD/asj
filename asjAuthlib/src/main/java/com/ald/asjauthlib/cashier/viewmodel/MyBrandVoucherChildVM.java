...
public class MyBrandVoucherChildVM extends BaseRecyclerViewVM<CouponListItemVM> {
    ...
    private MyBrandVoucherFragment fragment;

    private FragmentMyTicketsBrandListBinding binding;

    private int page = 0;

    private int menuType = Constant.VOUCHER_MENU_TYPE_BRAND;//优惠券类型,0:品台优惠券 1:品牌优惠券
    ...
    public MyBrandVoucherChildVM(MyBrandVoucherFragment fragment, int ticketType, FragmentMyTicketsBrandListBinding binding) {
        ...
        call.enqueue(new RequestCallBack<MyTicketListModel>() {
            ...
            public void onSuccess(Call<MyTicketListModel> call, final Response<MyTicketListModel> response) {
                ...
                if (MiscUtils.isNotEmpty(response.body().getCouponCenterUrl())) {
                    ...
                    }, ContextCompat.getColor(AlaConfig.getContext(),R.color.color_coupon_title_right_text));
                }
