...
import com.ald.asjauthlib.authframework.core.config.PermissionsResult;
import com.ald.asjauthlib.authframework.core.ui.CustomDialog;
import com.ald.asjauthlib.authframework.core.ui.PermissionRequestDialog;
...
public class PermissionCheck {
    ...
    //读取通讯录权限
    public static final int REQUEST_CODE_CCONTACT = 0x111;
...
//        return verify == PackageManager.PERMISSION_GRANTED;
    ...
    public void onRequestPermissionsResult(final Activity activity, int requestCode, @NonNull String[] permissions,
                                           @NonNull int[] grantResults, PermissionsResult permissionsResult) {
        if (requestCode == REQUEST_CODE_STORAGE || requestCode == REQUEST_CODE_PHONE || requestCode == REQUEST_CODE_SMS
                || requestCode == REQUEST_CODE_ALL || requestCode == REQUEST_CODE_CAMERA||requestCode==REQUEST_CODE_CCONTACT) {
                ...
                for (String permission : permissions) {
                    if (!ActivityCompat.shouldShowRequestPermissionRationale(activity, permission)) {
                        if ("android.permission.CAMERA".equals(permission)) {
                            showAskDialog(activity, R.string.permission_name_camera);
                        }
                        if ("android.permission.READ_CONTACTS".equals(permission)) {
                            showContactAskDialog(activity, R.string.permission_name_contacts);
                        }
                        ...
                        if ("android.permission.WRITE_EXTERNAL_STORAGE".equals(permission)) {
                            showAskDialog(activity, R.string.permission_name_sd);
                        }
                    }
                }
            }
        ...
        if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
            permissionsResult.onUIRequestPermissionsGrantedResult(requestCode);
        }
        ...
        new PermissionRequestDialog.Builder(activity).setTxtId(resId).setListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    intent.setData(Uri.parse("package:" + AlaConfig.getContext().getPackageName()));
