...
public class BitmapUtils {
    ...
    public static byte[] compress(Bitmap bitmap, int maxBytes) {
        ...
        try {
            ...
            Logger.e(TAG, "originSize:" + ret.length + ",max:" + maxBytes + ",float:" + mFloatBytes);
            if (ret.length < maxBytes) {
                Logger.e(TAG, "small enough ,return origin.");
                return ret;
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        while (true) {
            try {
                ...
                Logger.e(TAG, "compressed size:" + ret.length + ",center:" + ct);
                if (Math.abs(gap) <= mFloatBytes) {
                    Logger.e(TAG, "compressed OK:" + ret.length + ",center:" + ct + ",gap:" + gap);
                    return ret;
                } else {
                    if (gap > 0) {
                        ed = ct;
                        Logger.e(TAG, "too big,gap:" + gap + ",float " + mFloatBytes + ",try " + st + "-" + ed);
                    } else {
                        st = ct;
                        Logger.e(TAG, "too small,gap:" + gap + ",float " + mFloatBytes + ",try " + st + "-" + ed);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private static Bitmap getTailorBitmap(String path, int maxSize) {
        ...
        try {
            ...
            Logger.e("ESA", "getTailorBitmap decodeFile OutOfMemoryError." + error.getMessage());
            return null;
        }
        return bitmap;
    }
    ...
    public static Bitmap rotateBitmapByDegree(Bitmap bm, int degree) {
        ...
        try {
            ...
            Logger.e("ESA", "rotateBitmapByDegree decodeFile OutOfMemoryError." + error.getMessage());
        }
        if (bitmap == null) {
            bitmap = bm;
        }
