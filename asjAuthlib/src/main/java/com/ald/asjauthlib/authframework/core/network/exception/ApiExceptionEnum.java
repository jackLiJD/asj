...
public enum ApiExceptionEnum {
    ...
    REQUEST_EXCEEDED_LIMIT("REQUEST_EXCEEDED_LIMIT", 1018, "request exceeded limit", "请求次数过多，需要人机校验"),
...
//    ZM_CREDIT_SCORE_GET_ERROR("ZM_CREDIT_SCORE_GET_ERROR",1512,"zm credit score get error","调用芝麻信用评分失败"),
    ...
    ApiExceptionEnum(String errorMsg, int errorCode) {
        this.errorCode = errorCode;
        this.errorMsg = errorMsg;
    }

    private ApiExceptionEnum(String code, int errorCode, String errorMsg, String desc) {
        this.exception = code;
        this.errorCode = errorCode;
        this.errorMsg = errorMsg;
        this.desc = desc;
    ...
    public static ApiExceptionEnum getByCode(String code) {
        if (MiscUtils.isEmpty(code)) {
            return null;
        }
        ApiExceptionEnum[] errorCodes = values();

        for (ApiExceptionEnum acsErrorCode : errorCodes) {
