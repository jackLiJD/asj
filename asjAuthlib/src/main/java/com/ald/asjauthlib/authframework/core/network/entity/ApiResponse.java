package com.ald.asjauthlib.authframework.core.network.entity;
...
public class ApiResponse {
    ...
    public <T> T getData(Type cls) {
        if(data != null && MiscUtils.isNotEmpty(data.toJSONString())){
            return JSON.parseObject(data.toJSONString(), cls);
        }else {
            return null;
        }
...
//        return JSON.toJavaObject(json, cls);
    ...
    private <T> ApiPageData<T> parsePageResponse(JSONObject json, String entity, Class<T> cls) {
        ApiPageData<T> apiPageData = new ApiPageData<T>();
        apiPageData.setPageNo(json.getIntValue("pageNo"));
        if (json.containsKey("totalPage")) {
            apiPageData.setTotalPage(json.getIntValue("totalPage"));
        }
        if (json.containsKey("nextTimestamp")) {
            apiPageData.setNextTimestamp(json.getInteger("nextTimestamp"));
        }
