...
import com.ald.asjauthlib.tatarka.bindingcollectionadapter.support.IPositionItem;

import java.lang.ref.WeakReference;
import java.util.List;

import com.ald.asjauthlib.tatarka.bindingcollectionadapter.support.IBindItem;
 ...
 */
public class BindingRecyclerViewAdapter<T> extends RecyclerView.Adapter<ViewHolder> implements
    ...
    public void onBindBinding(ViewDataBinding binding, int bindingVariable, @LayoutRes int layoutRes, int position, T item) {
        if (bindingVariable != ItemView.BINDING_VARIABLE_NONE) {
            if (item instanceof IBindItem) {
                ((IBindItem) item).setBind(binding);
            }
            if (item instanceof IPositionItem) {
                ((IPositionItem) item).setPosition(position);
            }
